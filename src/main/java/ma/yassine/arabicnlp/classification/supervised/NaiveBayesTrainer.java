...
public class NaiveBayesTrainer {
    ...
    public static NaiveBayesModel trainWithSubset(Set<Integer> excluded, boolean invert) 
    {
        ...
        for (int d = 0; d < D; d++) {
            ...
            boolean isExcluded = excluded.contains(d);
            if (invert ? !isExcluded : isExcluded) continue;

            String label =
                LabelExtractor.extractLabel(TFIDFLoader.documents.get(d));

            classCounts.put(label,
                    classCounts.getOrDefault(label, 0.0) + 1);
            ...
            for (int t = 0; t < V; t++) {
                double val = TFIDFLoader.tfidfMatrix[d][t];
                if (val > 0) {
                    termCounts.get(label)
                        .put(t,
                            termCounts.get(label)
                            .getOrDefault(t, 0.0) + val);
                }
            }
        }
        ...
        Map<String, Double> priors = new HashMap<>();
        double totalDocs =
                classCounts.values().stream().mapToDouble(x -> x).sum();

        for (String c : classCounts.keySet()) {
            priors.put(c, Math.log(classCounts.get(c) / totalDocs));
        }
        ...
        for (String c : termCounts.keySet()) {
            Map<Integer, Double> probs = new HashMap<>();
            double sum =
                termCounts.get(c).values().stream().mapToDouble(x -> x).sum();
